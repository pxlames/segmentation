...
def getArgs():
    ...
    parser.add_argument('--end_epochs', type=int, default=500, metavar='N',
                        help='number of epochs to train [default: 500]')
    ...
    parser.add_argument('--dataset', default='DRIVE', type=str, help="BraTS/ISIC/LiTS/DRIVE/HC") #
    ...
    parser.add_argument('--test_epoch', type=int, default=500, metavar='N',
                        help='best epoch')
    # for ViT
    ...
    parser.add_argument(
        '--image-path',
        type=str,
        default='/home/pxl/myProject/血管分割/molong-深度插值/molong-work/DEviS-main/data/DRIVE/images/01_test.tif',
    ...
    parser.add_argument('--output-dir', type=str, default='/home/pxl/myProject/血管分割/molong-深度插值/molong-work/DEviS-main/results2/',
...
def getDataset(args):
    if args.dataset=='BraTS':
        ...
        root_path = '/home/pxl/myProject/血管分割/molong-深度插值/molong-work/DEviS-main/data/DRIVE/'
        exp_folder = args.folder
...
def test(args,model,test_loader):
    ...
    for i, data in enumerate(test_loader):
        ...
        x = input.cuda()
        # 保存输入图像
        input_img = torch.squeeze(x).cpu().numpy()[0] # 取第一个通道
        input_img = ((input_img - input_img.min()) / (input_img.max() - input_img.min()) * 255).astype(np.uint8)
        cv2.imwrite(os.path.join('/home/pxl/myProject/血管分割/molong-深度插值/molong-work/DEviS-main/results2/', str(i) + '_input.png'), input_img)
        ...
        with torch.no_grad():
            ...
            # 保存阈值化结果
            output = torch.squeeze(predicted).cpu().detach().numpy()
            uncertainty_numpy = torch.squeeze(uncertainty).cpu().detach().numpy()
            prob = torch.squeeze(prob).cpu().detach().numpy()[1]
            prob = (prob > 0.08).astype(np.float32)
            prob = ((prob - prob.min()) / (prob.max() - prob.min()) * 255).astype(np.uint8)
            '''这里挑选特定的u'''
            uncertainty_numpy = ((uncertainty_numpy - uncertainty_numpy.min()) / (uncertainty_numpy.max() - uncertainty_numpy.min()) * 255).astype(np.uint8)
            # 保存不确定性图
            cv2.imwrite(os.path.join('/home/pxl/myProject/血管分割/molong-深度插值/molong-work/DEviS-main/results2/', str(i) + '_uncertainty.png'), uncertainty_numpy)
            
            '''保存叠加结果'''
            # uncertainty_numpy叠加到input_img
            input_img_with_u = cv2.cvtColor(input_img.copy(), cv2.COLOR_GRAY2BGR)
            input_img_with_u[uncertainty_numpy==255] = [0, 0, 255] # BGR格式,设置为红色
            cv2.imwrite(os.path.join('/home/pxl/myProject/血管分割/molong-深度插值/molong-work/DEviS-main/results2/', str(i) + '_input_with_u.png'), input_img_with_u)
            # uncertainty_numpy叠加到prob
            prob_with_u = cv2.cvtColor(prob.copy(), cv2.COLOR_GRAY2BGR) 
            prob_with_u[uncertainty_numpy==255] = [0, 0, 255] # BGR格式,设置为红色
            cv2.imwrite(os.path.join('/home/pxl/myProject/血管分割/molong-深度插值/molong-work/DEviS-main/results2/', str(i) + '_prob_with_u.png'), prob_with_u)
            # cv2.imwrite(os.path.join(savepath, str(i) + '_pred.png'), prob)
            # 保存预测结果
            cv2.imwrite(os.path.join('/home/pxl/myProject/血管分割/molong-深度插值/molong-work/DEviS-main/results2/', str(i) + '_pred.png'), prob)

            # 保存GT图像
            gt = torch.squeeze(target).cpu().numpy()
            gt = ((gt - gt.min()) / (gt.max() - gt.min()) * 255).astype(np.uint8)
            cv2.imwrite(os.path.join('/home/pxl/myProject/血管分割/molong-深度插值/molong-work/DEviS-main/results2/', str(i) + '_gt.png'), gt)

            # U_output = torch.squeeze(uncertainty).cpu().detach().numpy()
            # U_output = ((U_output - U_output.min()) / (U_output.max() - U_output.min()) * 255).astype(np.uint8)
            # cv2.imwrite(os.path.join(savepath, str(i) + '_uncertainty.png'), U_output)
            ...
            if args.dataset == 'BraTS':
                ...
                if snapshot:
                    ...
                    #                 Noise_Snapshot_img)
                    # 确保target_img是uint8类型
                    target_img_uint8 = target_img.astype(np.uint8)
                    imageio.imwrite(os.path.join(savepath, name+'_gt.png'), target_img_uint8)
                    plt.figure()
                    plt.imshow(U_output, vmin=0, vmax=1, cmap='gray')
                    plt.colorbar()
                    plt.savefig(os.path.join(savepath, name+'_uncertainty.png'))
                    plt.close()
                    NU_output_uint8 = (NU_output * 255).astype(np.uint8)
                    imageio.imwrite(os.path.join(savepath, name+'_noised_uncertainty.png'), NU_output_uint8)

                    conf_output_uint8 = (conf_output * 255).astype(np.uint8)
                    imageio.imwrite(os.path.join(savepath, name+'_conf.png'), conf_output_uint8)
                    Nconf_output_uint8 = (Nconf_output * 255).astype(np.uint8)
                    imageio.imwrite(os.path.join(savepath, name+'_noised_conf.png'), Nconf_output_uint8)
                    confe_output_uint8 = (confe_output * 255).astype(np.uint8)
                    imageio.imwrite(os.path.join(savepath, name+'_confe.png'), confe_output_uint8)
                    Nconfe_output_uint8 = (Nconfe_output * 255).astype(np.uint8)
                    imageio.imwrite(os.path.join(savepath, name+'_noised_confe.png'), Nconfe_output_uint8)
                    ...
                    cv2.imwrite(
                        ...
                        NU_heatmap)
                    # 调整形状并转换类型
                    Oinput = Oinput.squeeze()  # 移除多余的维度
                    if Oinput.max() <= 1.0:    # 如果值在0-1范围内
                        Oinput = (Oinput * 255).astype(np.uint8)
                    else:
                        Oinput = Oinput.astype(np.uint8)

                    # 保存图像
                    Oinput_uint8 = (Oinput * 255).astype(np.uint8)
                    NOinput_uint8 = (NOinput * 255).astype(np.uint8)
                    imageio.imwrite(os.path.join(savepath, name+'_input.png'), Oinput_uint8)
                    imageio.imwrite(os.path.join(savepath, name+'_noised_input.png'), NOinput_uint8)
...
class ModelWrapper(torch.nn.Module):
    ...
    def forward(self, x):
        # 获取模型输出
        output = self.model(x, None, 0, 'test', args.dataset)
        # 选择前景类别
        output = output[0, 1, :, :]  # [1, 1, 584, 565]
        print(output.shape)
        
        # 不要转换为标量，保持原始维度
        # 但可以进行归一化处理
        # output = torch.sigmoid(output)  # 或者用softmax，取决于你的需求
        
        return torch.mean(output)  # 返回[1, 1, 584, 565]格式的张量
...
if __name__ == "__main__":
    ...
    # target_layers = [model.backbone.conv10]  # 最后的1x1卷积层
    # # # 如果想看多个层的CAM结果，可以这样写：
    # # target_layers = [
    # #     model.backbone.conv5.conv[-1],  # 编码器最深层
    # #     model.backbone.conv9.conv[-1],  # 解码器最后一层
    # #     model.backbone.conv10  # 最终输出层
    # # ]
