...
from models.topoloss_pd import TopoLossMSE2D
...
class TMSU(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.topo_weight = 0.0001
        self.topo_window = 128
        self.topo_loss = TopoLossMSE2D(self.topo_weight, self.topo_window)
    ...
    def forward(self, X, y, global_step, mode,dataset):
        ...
        if mode == 'train':
            ...
            # 将alpha转换为概率分布形式
            pred_prob = alpha / torch.sum(alpha, dim=1, keepdim=True)  # 归一化
            # topo_loss = self.topo_loss(pred_prob, y)
            # loss += topo_loss
