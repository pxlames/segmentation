...
def __surface_distances(result, reference, voxelspacing=None, connectivity=1):
    ...
    # test for emptiness
    if 0 == numpy.count_nonzero(result) and 0 == numpy.count_nonzero(reference):
        sds = numpy.ones(reference.shape)*1e-8
        # raise RuntimeError('The first supplied array does not contain any binary object.')
    elif 0 == numpy.count_nonzero(reference) and 0 != numpy.count_nonzero(result):
        sds = numpy.ones(reference.shape)*1e-8
        # raise RuntimeError('The second supplied array does not contain any binary object.')
    elif 0 != numpy.count_nonzero(reference) and 0 == numpy.count_nonzero(result):
        sds = numpy.ones(reference.shape) * 100
    else:
