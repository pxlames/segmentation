...
def warping_loss(y_pred, y_gt):
    ...
    loss = ce_loss(y_pred * torch.unsqueeze(torch.from_numpy(critical_points), dim=1), y_gt * torch.unsqueeze(torch.from_numpy(critical_points), dim=1)) * len(np.nonzero(critical_points)[0])
    return loss
...
# 读取图像
gt_path = '/home/pxl/myProject/血管分割/FIVE的分割GT.png'
pred_path = '/home/pxl/myProject/血管分割/糟糕的预测图.png'
gt_img = cv2.imread(gt_path, cv2.IMREAD_GRAYSCALE)
pred_img = np.asarray(cv2.imread(pred_path, cv2.IMREAD_GRAYSCALE))
gt_img = torch.Tensor(gt_img).unsqueeze(0).unsqueeze(0)
pred_img = torch.Tensor(pred_img).unsqueeze(0).unsqueeze(0)
# 测试wrapLoss函数

loss_topo = warping_loss(pred_img, gt_img)
print(loss_topo)
