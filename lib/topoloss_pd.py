...
class TopoLossMSE2D(torch.nn.Module):
    ...
    该类实现了一个加权的拓扑损失函数,可以与PyTorch模型配合使用。
    用于计算预测结果与目标之间的拓扑差异。
    """
...
#  计算预测图像（likelihood）的0维持久性同调特征，提取出生点和死亡点以及相应的临界点。
def getCriticalPoints_cr(likelihood):
        
    lh = 1 - likelihood
    pd = cr.computePH(lh, maxdim=1, location="birth") # 计算持久图
    pd_arr_lh = pd[pd[:, 0] == 0] # 0-dim topological features # 提取0维拓扑特征（连通分量）
...
# 计算两个持久图之间的Wasserstein距离，并匹配持久点。
def get_matchings(lh, gt):
    ...
    # 返回需要与对角线匹配的点和与目标特征匹配的点
    return dgm_to_diagonal, off_diagonal_match

# 计算持久图之间的匹配，并根据地面真实图像的持久特征来确定哪些持久点需要调整。
...
def getTopoLoss(pred_tensor, gt_tensor, topo_window):
    ...
    topo_cp_weight_map = np.zeros(likelihood.shape) # 标记需要优化的位置
    topo_cp_ref_map = np.zeros(likelihood.shape) # 指导特征点移动的目标位置
    # 分块处理图像: 遍历图像，以topo_window为步长，逐块计算拓扑损失
    for y in range(0, likelihood.shape[0], topo_window):
        for x in range(0, likelihood.shape[1], topo_window):
            ...
            # 特征匹配: 计算持久图之间的匹配，并根据地面真实图像的持久特征来确定哪些持久点需要调整。
