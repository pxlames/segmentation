...
def CI_fwer(scores, masks, alpha=0.05):
    ...
    masks (ndarray): 3D binary array i1ndicating the mask for each image, matching the scores array.
...
# 使用方法
# threshold_func = create_lamhat_threshold_FPR(cal_scores, cal_masks, cal_scores.shape[0], 1)
...
saveTrainDir = '/home/xkw/pxlames/segmentation/outputs/firstStageResults/FIVE-BceDiceSmooth_0.0001_4/FIVE3/train/Cfolder'
saveValDir = '/home/xkw/pxlames/segmentation/outputs/firstStageResults/FIVE-BceDiceSmooth_0.0001_4/FIVE3/val/Cfolder'
saveTestDir = '/home/xkw/pxlames/segmentation/outputs/firstStageResults/FIVE-BceDiceSmooth_0.0001_4/FIVE3/test/Cfolder'

saveTrainDir2 = '/home/xkw/pxlames/segmentation/outputs/firstStageResults/FIVE-BceDiceSmooth_0.0001_4/FIVE3/train/Ufolder'
saveValDir2 = '/home/xkw/pxlames/segmentation/outputs/firstStageResults/FIVE-BceDiceSmooth_0.0001_4/FIVE3/val/Ufolder'
saveTestDir2 = '/home/xkw/pxlames/segmentation/outputs/firstStageResults/FIVE-BceDiceSmooth_0.0001_4/FIVE3/test/Ufolder'


def remove_minarea(img, min_area):
    """
    去除图像中面积小于指定阈值的连通分量
    
    参数:
    img: 二值图像, uint8类型，背景为0，前景为1或255
    min_area: 最小面积阈值，小于此面积的连通分量将被移除
    ...
    # 查看输入图像的值范围
    print(f"Input image value range: min={output.min()}, max={output.max()}")
    
    # 确保图像是二值的
    if output.max() == 1:
        binary_img = output
    else:
        _, binary_img = cv2.threshold(output, 127, 1, cv2.THRESH_BINARY)
    ...
    # 遍历所有连通分量（跳过背景，即labels=0的部分）
    for i in range(1, num_labels):
        area = stats[i, cv2.CC_STAT_AREA]
        if area < min_area:
            # 将小于阈值的连通分量置为0
            output[labels == i] = 0
            print(f"Removing component {i} with area {area}")
...
for i in range(test_scores.shape[2]):  
    ...
    test_result_U_r = remove_minarea(test_result_U,min_area=2000)
    # 保存图像
    ...
    img = Image.fromarray(test_result_U_r)
...
for i in range(train_scores.shape[2]):  
    ...
    train_result_U_r = remove_minarea(train_result_U,min_area=2000)
    ...
    # # 轻微腐蚀处理去除边缘细线
    # processed_img = remove_edge_lines(diff_img)
    img = Image.fromarray(train_result_U_r)
...
for i in range(val_scores.shape[2]):  
    ...
    val_result_U_r = remove_minarea(val_result_U,min_area=2000)
    ...
    # 轻微腐蚀处理去除边缘细线
    # processed_img = remove_edge_lines(diff_img)
    img = Image.fromarray(val_result_U_r)
